...
Les deux joueurs n’utilisent pas toutes leurs stratégies de manière uniforme.  
Chacun joue uniquement les stratégies du support optimal
\(\{2,4,6\}\) avec probabilité \(1/3\) chacune.

La résolution numérique donne la valeur du jeu :
\[
v = \frac{1}{3}.
\]

Comme \(v > 0\), le jeu avantage le joueur \(X\) : il peut garantir une espérance
de gain de \(1/3\) contre toute stratégie du joueur \(Y\), tandis que \(Y\) ne peut
pas réduire cette espérance en dessous de \(1/3\).

Les stratégies mixtes optimales sont :
\[
X^{*} =
\left(
0,\;
...
Ces stratégies constituent un point selle en stratégies mixtes, et la valeur du jeu
est \(v = \frac{1}{3}\).
