...
X = (x_1,\ldots,x_6)
\)
et une valeur \(g\) telles que, pour chaque stratégie pure \(j\) de B,
\[
E(X,j) = \sum_{i=1}^{6} a_{ij} x_i \;\ge\; g.
\]

Le système d'inégalités correspondant, pour notre matrice de jeu, est :
\[
\begin{cases}
...
Le joueur X choisit une configuration et le joueur Y propose (simultanément) une case parmi les 6. 

Si la configuration recouvre cette case Y gagne sinon X gagne. 

C'est un jeu somme nulle (si la valeur est $> 0$, c'est un gain pour $X$ et une perte pour $Y$) docn si Jouur X gagne +1 point joeur Y perdre -1 point. Le somme de points gagnent et points de perdre est toujours pareils.


\subsection{Modelisation}
Il y a 7 strategies different pour X donc il y a 7 ligne.

Le premier 3 collone represante 1ier ligne de jeu.
Le dernier 3 collone represante 2ieme ligne de jeu

Les cases cohisi par le X ont une valuer 1.
...
Comme tous les minima des lignes valent $-1$, on obtient :
\[
V^- = \max(-1,-1,-1,-1,-1,-1) = -1.
\]

Comme tous les maxima des colonnes valent $1$, on obtient :
\[
V^+ = \min(1,1,1,1,1,1) = 1.
\]
